...
8. 默认 failurePolicy 保持 stop。只有需要收集多个独立校验失败时才显式设置 continue。轮询一致性使用 retryUntil，不要用固定 sleep。
9. 不把真实生产地址、真实个人数据、非测试凭据、固定 Token 或项目外部依赖写入公共示例。不要修改业务代码、构建配置、公共 scenario-test 运行时或已有场景的语义。
10. 最后输出：新增/修改的场景文件、每个场景覆盖的流程、仍待确认的接口或测试数据条件，以及本地运行命令。不要实际执行场景。
```

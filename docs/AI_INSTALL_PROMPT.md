...
5. init 会保留已有文件。检查并报告以下文件是否已创建或已保留：
   ...
   - scenario-test/scenario-test.d.ts
   - scenario-test/scenario-test-capabilities.json
   - scenario-test/.scenario-test-version.json
...
安装完成后，继续使用 [AI 场景生成 Prompt](AI_SCENARIO_PROMPT.md) 生成项目业务用例。DSL 能力名单以 `node scenario-test/scenario-test-cli.cjs capabilities` 与 `scenario-test-capabilities.json` 为准，不要手工抄写多份文档。

...
3. 默认免 npm 安装：直接执行官方固定版本脚本（Windows: irm https://cdn.jsdelivr.net/gh/youzhikeji/scenario-test@v0.5.18/scripts/install.ps1 | iex；macOS/Linux: curl -fsSL https://cdn.jsdelivr.net/gh/youzhikeji/scenario-test@v0.5.18/scripts/install.sh | bash -s -- . scenario-test）。只有用户明确要求使用 npm 时，才改用 npm install -D @yc_yzkj/scenario-test 并执行 npx @yc_yzkj/scenario-test init --project . --dir "scenario-test"。
...
```powershell
# Windows PowerShell
irm https://cdn.jsdelivr.net/gh/youzhikeji/scenario-test@v0.5.18/scripts/install.ps1 | iex
```

```bash
# macOS / Linux
curl -fsSL https://cdn.jsdelivr.net/gh/youzhikeji/scenario-test@v0.5.18/scripts/install.sh | bash -s -- . scenario-test
```
...
在业务项目根目录执行（版本号换成目标版本，当前 `v0.5.18`）：
...
```powershell
npm install -D @yc_yzkj/scenario-test@0.5.18
npx @yc_yzkj/scenario-test init --project . --dir "scenario-test"
```
...
```powershell
.\install.ps1 -Source "https://gitlab.example.com/group/project/-/raw/v0.5.18/dist"
```
